...
if __name__ == "__main__":
    ...
    if args.phase == 'test':
        ...
        for sampled_batch in tqdm(testloader, ncols=70):
            ...
            io.imsave(save_path + str(cnt) + '_ct.png', bright(ct_img,0,0.8))

            ct_in_img = (ct_in.data.cpu().numpy()[0, 0] * 255).astype(np.uint8)
            io.imsave(save_path + str(cnt) + '_ct_in.png', bright(ct_in_img,0,0.8))

            mri_img = (mri.data.cpu().numpy()[0, 0] * 255).astype(np.uint8)
            io.imsave(save_path + str(cnt) + '_mri.png', bright(mri_img,0,0.8))

            mri_in_img = (mri_in.data.cpu().numpy()[0, 0] * 255).astype(np.uint8)
            io.imsave(save_path + str(cnt) + '_mri_in.png', bright(mri_in_img,0,0.8))
            ...
            for idx, lam in enumerate([0,0.3,0.5,0.7,1.0]):
                ...
                io.imsave(save_path + str(cnt) + '_fusion_' + str(lam) + '.png', bright(fusion_img,0,0.8))
                ii = ii + 1
